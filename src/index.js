...
export default class CanvasDraw extends PureComponent {
  ...
  componentDidMount() {
    ...
    window.setTimeout(() => {
      ...
      this.lazy.update({ x: initX / 4, y: initY }, { both: true })
      this.lazy.update({ x: initX / 4, y: initY }, { both: false })
      this.mouseHasMoved = true
      this.valuesChanged = true
      this.clearExceptErasedLines()
  ...
  componentDidUpdate(prevProps) {
    ...
    this.coordSystem.scaleExtents = { min: 0.33, max: 3 }
  ...
  handleWheel = (e) => (this.interactionSM = this.interactionSM.handleMouseWheel(e, this))
  ...
  handleDrawMove = (e) => {
    if (this.props.drawable) this.interactionSM = this.interactionSM.handleDrawMove(e, this)
    else this.interactionSM = this.interactionSM.handleDrawEnd(e, this)
    this.mouseHasMoved = true
  }
