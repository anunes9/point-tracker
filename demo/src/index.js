    ...
    return (
      <div>
        ...
        <p>
          ...
          <span>{'<CanvasDraw brushColor={this.state.color} />'}</span>. We randomly change them every 2 seconds. But
          you could easily use a color-picker!
        </p>
        <div>
          Current color:{' '}
          <div
        ...
        <CanvasDraw brushColor={this.state.color} drawable={false} />
        ...
        <h2>Zoom & Pan</h2>
        <p>
          Set the <span>enablePanAndZoom</span> prop to enable mouse scrolling and panning (using Ctrl), pinch zooming,
          and two-finger panning. If you want to ensure that all lines stay within the bounds of the canvas, set the{' '}
          <span>clampLinesToDocument</span> property.
        </p>
        <CanvasDraw
          enablePanAndZoom
        ...
        <h2>Save & Load</h2>
        <p>
          This part got me most excited. Very easy to use saving and loading of drawings. It even comes with a
          customizable loading speed to control whether your drawing should load instantly (loadTimeOffset = 0) or
          appear after some time (loadTimeOffset &gt; 0) <span>{'<CanvasDraw loadTimeOffset={10} />'}</span>
        </p>
        ...
        />
        <p>The following is a disabled canvas with a hidden grid that we use to load & show your saved drawing.</p>
        <button
          onClick={() => {
            this.loadableCanvas.loadSaveData(localStorage.getItem('savedDrawing'))
          }}
        >
          Load what you saved previously into the following canvas. Either by calling `loadSaveData()` on the
          component's reference or passing it the `saveData` prop:
        </button>
        <CanvasDraw
          disabled
          hideGrid
        ...
        />
        <p>
          The saving & loading also takes different dimensions into account. Change the width & height, draw something
          and save it and then load it into the disabled canvas. It will load your previously saved masterpiece scaled
          to the current canvas dimensions.
        </p>
        <p>
          That's it for now! Take a look at the{' '}
          <a href="https://github.com/mBeierl/react-canvas-draw/tree/master/demo/src">source code of these examples</a>.
        </p>
      </div>
    )
  }
